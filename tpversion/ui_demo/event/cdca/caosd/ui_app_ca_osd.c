...
char __glb_caosd_show_osdMesage[180+60+1];
char __glb_caosd_round_show[MSG_LEN_IN_LINE+1];
BU32 __glb_caosd_show_osd_i=0;

char __glb_caosd_show_osdMesageTop[180+60+1];
...
{	
	...
	__glb_caosd_show.bu32ShowInfo = 1;
  	...
  	APP_Printf("__glb_caosd_show.bu32ShowInfo = 1, start osd show\n");
	memset(__glb_caosd_show_osdMesage, 0, 241);//clear
...
{	
	...
	__glb_caosd_show.bu32ShowInfo_top= 1;
  	...
  	APP_Printf("__glb_caosd_show.bu32ShowInfo = 1, start osd show\n");
	memset(__glb_caosd_show_osdMesageTop, 0, 241);//clear
...
{
	/* 永新视博显示指纹
	   dwCardID == 0:取消指纹显示;dwCardID != 0:指纹显示
	   wDuration:要求显示指纹的Service的EcmPid	   
	*/	
...
void _CA_OSD_ShowPrinting(BU32 hdc)
{
	...
	offset_x = 320;//GUI_API_Random(600);
	...
	if(__glb_caosd_show.bu32CardID)
	...
	//	if(__glb_caosd_show.bu32IDCnt > 1)
		{  
			...
			if(__glb_caosd_show.printDuration == chnInfo.m_EcmPID || __glb_caosd_show.printDuration == chnInfo.m_audioEcmPID || __glb_caosd_show.printDuration == chnInfo.m_videoEcmPID)
			{
...
void _CA_OSD_ShowMessage(BU32 hdc)
{
	...
	{
	//	GUI_API_FillRect(hdc, __glb_caosd_show.bu32XPos, __glb_caosd_show.bu32YPos, 720, 30, 255);
		__glb_caosd_show_osd_i = 0;
	//	time_end = FYF_API_time_ms();
...
/*--------osd_msg top---------*/
	...
	case 6:
		...
		break;
	case 7:
/*--------detitle received notify---------*/
		if(UI_APP_API_IsOsdShow ())
		{
			return;
		}
		_CA_OSD_ShowDetitleReceived (hdc);
		break;
	case 8:
		if(UI_APP_API_IsOsdShow ())
		{
...
//	if(ui_detitle_received_info.show)
	{
		GUI_ENC_API_Update (ID_ODC_CaOsd_ChInfo, 7);
		ui_detitle_received_info.show = 0;
	}
}
