...
static BU08 g_ca_startsecd=0;
static BU08 g_ca_endHour=23;
static BU08 g_ca_endMin=59;
static BU08 g_ca_endsecd=59;
...
void UI_APP_CA_WorkTimeSet_Enter(void)
{
	...
	if(FYF_OK == ret)
	{
		...
		g_ca_startsecd = workTime.byStartSecond;
		g_ca_endHour = workTime.byEndHour;
		g_ca_endMin = workTime.byEndMinute;
		g_ca_endsecd = workTime.byEndSecond;
	}
...
{
	BU08 showStr[6]={0};
	sprintf(showStr,"%02d:%02d:%02d",g_ca_startHour,g_ca_startMin,g_ca_startsecd);
	if(GUI_ENC_API_IsFocus(ID_ODC_StartTime_WorkTime))
	{
		...
		else if(g_ca_offset>=4&&g_ca_offset<6)
		{
			GUI_API_DrawHLine(hdc, WORKTIME_X+(g_ca_offset+2)*WORKTIME_W, 30, WORKTIME_W, COLOR_BLACK);
		}
...
{
	BU08 showStr[6]={0};
	sprintf(showStr,"%02d:%02d:%02d",g_ca_endHour,g_ca_endMin,g_ca_endsecd);
...
void UI_APP_CA_WorkTimeSet_Key(BU32 key)
{
	...
	case GUI_VK_9:
		...
		if(GUI_ENC_API_IsFocus(ID_ODC_StartTime_WorkTime))
		{
			...
			case 3:
				...
				break;
			case 4:
				g_ca_startsecd = (BU08)((BU16)g_ca_startsecd%10 + key*10);
				break;
			case 5:
				g_ca_startsecd = (BU08)((BU16)g_ca_startsecd-g_ca_startsecd%10 + key);
				break;
			default:
				break;
			}
			...
			if(g_ca_startsecd>=60)g_ca_startsecd = 0;
			g_ca_offset++;
			if(g_ca_offset>5)g_ca_offset=0;
		...
		else
		{
			...
			case 3:
				...
				break;
			case 4:
				g_ca_endsecd = (BU08)((BU16)g_ca_endsecd%10 + key*10);
				break;
			case 5:
				g_ca_endsecd = (BU08)((BU16)g_ca_endsecd-g_ca_endsecd%10 + key);
				break;
			default:
				break;
			}
			...
			if(g_ca_endsecd>=60)g_ca_endsecd = 59;
			g_ca_offset++;
			if(g_ca_offset>5)g_ca_offset=0;
	...
	case GUI_VK_LEFT:
		...
		{
			if(g_ca_offset==0)g_ca_offset=5;
			else g_ca_offset--;
			if(GUI_ENC_API_IsFocus(ID_ODC_StartTime_WorkTime))
				GUI_ENC_API_Update(ID_ODC_StartTime_WorkTime, 1);
	...
	case GUI_VK_SELECT:
		...
		workTime.byStartSecond= g_ca_startsecd;
		workTime.byEndHour = g_ca_endHour;
		workTime.byEndMinute = g_ca_endMin;
		workTime.byEndSecond= g_ca_endsecd;
		ret = FYF_API_ca_set_value(FYF_CA_SV_WORK_TIME,(BU32)&workTime,(BU32)g_ca_oldPin);
		if(FYF_OK == ret)
		{
		...
		else
		{
			...
			if(FYF_RC_WORKTIME_INVALID == ret)
				GUI_ENC_API_SetValue(ID_STA_PinNote,WDT_STA_SV_DATAID,RS_TimeInvalid,0);
			else if(FYF_RC_PIN_INVALID)
