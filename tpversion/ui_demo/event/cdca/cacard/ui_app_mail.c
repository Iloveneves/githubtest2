...
static BU08 g_mail_delAll = 0;

static FYF_CA_EmailHead_s g_mail_head[100];
static FYF_CA_EmailIdx_s  g_mail_index;
...
void UI_APP_Mail_ShowCurPage(void)
{
	...
	for(index = 0;index<G_MAIL_NUM_IN_PAGE;index++)
	{
		...
		if(itemIndex<g_mail_allCount)
		{
			...
			//sprintf(g_mail_item_str[index],"%s %-2d %-18s %-12s",newStr,itemIndex+1,CreateTime,titleTmp);
			sprintf(g_mail_item_str[index]," %-3d %-11s %-22s %-6s %s",itemIndex+1,titleTmp,CreateTime,newStr,(g_mail_head[itemIndex].m_wImportance)?pDic[RS_Important]:pDic[RS_Normal]);
			itemNum++;
		}
	}
...
void UI_APP_CA_Mail_Enter(void)
{
	...
	FYF_API_Memset(g_mail_head, 0, sizeof(FYF_CA_EmailHead_s) * 100);
	FYF_API_Memset(&g_mail_index, 0, sizeof(FYF_CA_EmailIdx_s));
	g_mail_index.m_Count = 100;
	FYF_API_ca_get_value(FYF_CA_SV_EMAIL_COUNT,(BU32*)g_mail_head,(BU32*)&g_mail_index);
	realMailCount = g_mail_index.m_Count;
	UI_APP_CA_Mail_Reverser(g_mail_head, realMailCount);
	for(i=0;i<realMailCount;i++)
	{
		if(g_mail_head[i].m_bNewEmail)
			newCount++;
	}
	...
	else
	{
		...
		g_mail_allCount = (realMailCount>100)?100:realMailCount;
...
void UI_APP_CA_Mail_Change(void)
{
	...
	UI_APP_Mail_ShowCurPage();//ÏÔÊ¾µ±Ç°Ò³
...
void UI_APP_CA_ReadMail_Enter(void)
{
	...
	/*level*/
	if(g_mail_head[itemIndex].m_wImportance)
	{
		sprintf(g_ui_app_str[2],"%s: %s",pDic[RS_Level],pDic[RS_Important]);
	}
	else
	{
		sprintf(g_ui_app_str[2],"%s: %s",pDic[RS_Level],pDic[RS_Normal]);	
	}
	GUI_ENC_API_SetValue(ID_STA_Line3_ReadMail, WDT_STA_SV_STRING, (BU32)g_ui_app_str[2], 0);
	
	GUI_API_Memset(g_mail_content.m_szEmail, 0, 161);
	FYF_API_ca_get_value(FYF_CA_SV_MAIL_BODY,(BU32*)&(g_mail_head[itemIndex].m_dwActionID),(BU32*)&g_mail_content);
...
void UI_APP_CA_DelMail_Key(BU32 key)
{
	...
	if(key == GUI_VK_SELECT)
	{
		...
		{
			temp = 0;
			FYF_API_ca_set_value(FYF_CA_SV_EMAIL_DEL,temp,0);
			GUI_ENC_API_SwitchState(ID_DLG_Mail, 1);
		}
		else
		{
