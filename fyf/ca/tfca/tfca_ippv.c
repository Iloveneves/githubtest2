...
//-------------------- IPPV应用 --------------------
void TFSTBCA_HideIPPVDlg(CDCA_U16 wEcmPid)
{
	CDSTBCA_IPP_PROTECT ();
...
void TFSTBCA_StartIppvBuyDlg(CDCA_U8 byMessageType, CDCA_U16 wEcmPid,  SCDCAIppvBuyInfo *pIppvProgram)
{
	CDSTBCA_IPP_PROTECT ();
...
void UI_APP_FullScreen_ipp_price_Key(BU32 key)
{
	...
	{	
		if(GUI_VK_EXIT == key)
		{
			TFSTBCA_HideIPPVDlg (CDCAS_ipp.ecmPid);
			/* 退出订购 */
			TFCASTB_StopIPPVBuyDlg(CDCA_FALSE,CDCAS_ipp.ecmPid,NULL,NULL);
		}
	...
	else if(GUI_ENC_API_IsFocus(ID_TBX_ipp_pin_ChnInfo))
	{
		...
		{			
			...
			if(!FYF_API_Strcmp (str, "......") || FYF_API_Strlen (str) != 6)
			{
				...
				{
					sprintf (g_cdcas_str[10], "(%s)", "输入PIN有误");
				}				
				GUI_ENC_API_SetValue (ID_STA_ipp_pin_prompt_ChnInfo,WDT_STA_SV_STRING,(BU32)g_cdcas_str[10],0);
				GUI_ENC_API_SetValueUnPaint (ID_TBX_ipp_pin_ChnInfo,WDT_TBX_SV_PASSWORD,(BU32)password,0);
			}
			else
			{
				...
				CDCAS_ipp.retValue = TFCASTB_StopIPPVBuyDlg(CDCA_TRUE,CDCAS_ipp.ecmPid,pin,&(CDCAS_ipp.ippvProgram.m_Price[index]));
