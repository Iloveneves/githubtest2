	...
	for (i = 0; i < CA_REQID_NUM; i++)
	{
		if (CAReqID[i].u8runflag == 1 && CAReqID[i].u32waittime != 1000*1000)
		{
			...
			if (times - CAReqID[i].u32tick > CAReqID[i].u32waittime)
			{
				...
				TFCASTB_PrivateDataGot(i, false, pid, NULL, 0 );	
...
//EMM数据 长度小于16，丢弃
	...
	TFCASTB_PrivateDataGot(i, sectionflag, pid, u8Buffer, u32BufferLength );
	CAS_DMX_MUTEX_UNLOCK();
...
BS32 cas_demux_request(BU08 req_id, BU08* value, BU08* mask, BU08 mask_len, BU16 pid, BU08 wait_seconds)
{
	...
	if (req_id < 2)
	{
		APP_Parser_IPI_StopDemux(&(CAReqID[req_id].u32ChannelID), &(CAReqID[req_id].u32FilterID));
		APP_Parser_IPI_StartDemux(mask_len, pid, value, mask, excl, &(CAReqID[req_id].u32ChannelID), &(CAReqID[req_id].u32FilterID));
	}
	else
	{
		...
		{
			if (pid != CAReqID[req_id].u16tspid)
			{
				APP_Parser_IPI_StopDemux(CAReqID[req_id].u32ChannelID, CAReqID[req_id].u32FilterID);
			}
	...
	CAReqID[req_id].u16reqid = req_id;
	CAReqID[req_id].u16tspid = pid;
	CAReqID[req_id].u8runflag = 1;
...
BS32 cas_demux_request(BU08 req_id, BU08* value, BU08* mask, BU08 mask_len, BU16 pid, BU08 wait_seconds)
{
	...
	CAReqID[req_id].u16tspid = pid;
	CAReqID[req_id].u8runflag = 1;
	CAReqID[req_id].u32tick = FYF_API_time_ms();
	if (wait_seconds == 0)
		CAReqID[req_id].u32waittime = 1000*1000;
	else 
		CAReqID[req_id].u32waittime = wait_seconds*1000;
...
-------------------------------------------------------------------------------*/
CDCA_BOOL TFSTBCA_GetPrivateData(CDCA_U8 byReqID, const CDCA_U8* szFilter, const CDCA_U8* szMask, CDCA_U8 byLen, CDCA_U16 wPID, CDCA_U8 nWaitSeconds)
...
-------------------------------------------------------------------------------*/
void	TFSTBCA_ScrSetCW(CDCA_U16 wEcmPID, const CDCA_U8* pbyOddKey, const CDCA_U8* pbyEvenKey, CDCA_U8 byKeyLen, CDCA_BOOL bTapingEnabled)
{
	BU08 CA_CW[8];
	...
	DEBUG_CDCA("TFSTBCA_ScrSetCW byKeyLen = %d\n",byKeyLen);
	for (i = 0; i < 2; i++)
	{
