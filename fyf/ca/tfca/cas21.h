...
-*/
extern CDCA_BOOL  TFCASTB_Init(CDCA_U8 byThreadPrior);

/*++
 * 这个函数将被用来判断是否是CASYSTEMID能够被当前CA识别	
-*/
//extern CDCA_BOOL  TFCASTB_IsCDCa(CDCA_U16 wCaSystemID);
extern CDCA_BOOL  TFCASTB_IsTfCa(CDCA_U16 wCaSystemID);
...
-*/
extern void  TFCASTB_FormatBuffer(CDCA_U8 byFormatType);
extern void  TFCASTB_RequestMaskBuffer(void);
extern void  TFCASTB_RequestUpdateBuffer(void);
...
/*++
	if(用户切换频道||或者当前频道中ECMPID发生增加或者减少)
	{
		...
		if(如果当前频道中有ECMPID,既节目是加扰的) //有几个ECMPID送几次，ECMPID相同送一次就可以
			TFCASTB_SetEcmPid(CDCA_LIST_ADD,pServiceInfo);
		TFCASTB_SetEcmPid(CDCA_LIST_OK,NULL);
	}
--*/
extern void  TFCASTB_SetEcmPid(CDCA_U8 byType,  SCDCASServiceInfo* pServiceInfo);
extern void  TFCASTB_SetEmmPid(CDCA_U16 wEmmPid);
extern void  TFCASTB_PrivateDataGot(CDCA_U8 byReqID, CDCA_BOOL bOK, CDCA_U16 wPid, const CDCA_U8* pbyReceiveData, CDCA_U16 wLen);

//-------------------- 智能卡管理 --------------------
extern CDCA_BOOL  TFCASTB_SCInsert(void);
...
例子：
	...
	CDCA_U16	wRet = 0;
	wRet= TFCASTB_GetCardSN(sn);
	if(wRet==CDCA_RC_OK)
		printf("SN:%s",sn);
	}
 */
extern CDCA_U16  TFCASTB_GetCardSN(char* pCardSN);

extern CDCA_U16  TFCASTB_ChangePin( const CDCA_U8* pbyOldPin, const CDCA_U8* pbyNewPin);
/*++

级别说明：
   byRating的范围是[4,18],是年龄 
*/
extern CDCA_U16  TFCASTB_SetRating(const CDCA_U8* pbyPin, CDCA_U8 byRating);
extern CDCA_U16  TFCASTB_GetRating(CDCA_U8* pbyRating);

extern CDCA_U16  TFCASTB_SetWorkTime(const CDCA_U8* pbyPin, CDCA_U8 byStartHour, CDCA_U8 byStartMin, CDCA_U8 byStartSec, CDCA_U8 byEndHour, CDCA_U8 byEndMin, CDCA_U8 byEndSec);
extern CDCA_U16  TFCASTB_GetWorkTime(CDCA_U8* pbyStartHour, CDCA_U8* pbyStartMin, CDCA_U8* pbyStartSec, CDCA_U8* pbyEndHour, CDCA_U8* pbyEndMin, CDCA_U8* pbyEndSec);
//-------------------- 基本信息查询 --------------------
extern CDCA_U32 TFCASTB_GetVer(void);
/*++
说明 ：
...
例子：
	...
	if(TFCASTB_GetOperatorIds(wArrTvsID) == CDCA_RC_OK)
	{
		for(index=0,index<CDCA_MAXNUM_OPERATOR;index++)
		{	
			if(wArrTvsID[index]==0)
				break;
			if(TFCASTB_GetOperatorInfo(wArrTvsID[index],&OperatorInfo) == CDCA_RC_OK)
			{
...
--*/
extern CDCA_U16 TFCASTB_GetOperatorIds(CDCA_U16* pwTVSID);
extern CDCA_U16 TFCASTB_GetOperatorInfo(CDCA_U16 wTVSID,  SCDCAOperatorInfo* pOperatorInfo);
/*++
说明 ：
...
例子：
    ...
    if(TFCASTB_GetACList(wOperatorID,ACArray) == CDCA_RC_OK)
	{
		printf("Get operator AC OK!);
	}
--*/
extern CDCA_U16 TFCASTB_GetACList(CDCA_U16 wTVSID, CDCA_U32* pACArray);
/*++
说明 ：
	获得某一个运营商的授权信息
例子：
	 SCDCAEntitles Entitle;
        ...
        if(wRet == CDCA_RC_OK)
        {
                ...
                printf("相应的错误");
*/
extern CDCA_U16 TFCASTB_GetServiceEntitles(CDCA_U16 wTVSID, SCDCAEntitles* pServiceEntitles);
...
例子：
         ...
         wRet = TFCASTB_GetEntitleIDs(wOperatorID,EntitleIds);
         if(wRet == CDCA_RC_OK)
         {
          CDCA_U16 wIndex=0;
          ...
          printf("相应的错误");
*/
extern CDCA_U16 TFCASTB_GetEntitleIDs(CDCA_U16 wTVSID, CDCA_U32* pdwEntitleIds);

extern CDCA_U16  TFCASTB_GetDetitleChkNums(CDCA_U16 wTvsID,  CDCA_U32 *pdwDetitleChkNums);
extern CDCA_BOOL TFCASTB_GetDetitleReaded(CDCA_U16 wTvsID);
extern CDCA_BOOL TFCASTB_DelDetitleChkNum(CDCA_U16 wTvsID, CDCA_U32 dwDetitleChkNum);
...
例子：
 				CDCA_U16  wRet = 0;
        wRet = TFCASTB_IsPaired(pbyNum, pbySTBID_List);
        if(wRet == CDCA_RC_OK)
...
--*/
extern CDCA_U16 TFCASTB_IsPaired(CDCA_U8* pbyNum, CDCA_U8* pbySTBID_List);
extern CDCA_U16 TFCASTB_GetPlatformID(void);
...
//-------------------- IPPV应用 --------------------
	...
	例子：
        ...
        if(TFCASTB_GetSlotIDs(1,byArrSlotID) == CDCA_RC_OK)
        {
	...
	--*/
	extern CDCA_U16 TFCASTB_GetSlotIDs(CDCA_U16 wTVSID,  CDCA_U8* pbySlotID);
	extern CDCA_U16 TFCASTB_GetSlotInfo(CDCA_U16 wTVSID, CDCA_U8 bySlotID, SCDCATVSSlotInfo* pSlotInfo);
	/*++
	说明 ：
		获得一个运营商的所有的IPPV节目
	例子：
        ...
        if(TFCASTB_GetIPPVProgram(1,ArrIppv,&wNumber) == CDCA_RC_OK)
        {
	...
	--*/
	extern CDCA_U16 TFCASTB_GetIPPVProgram(CDCA_U16 wTVSID,  SCDCAIppvInfo* pIppv, CDCA_U16* pwNumber);
	/*++
	说明 ：
	...
	例子：
	   在IPPV提示购买的对话框显示在电视上的时候
        ...
        if(用户选择购买)
        {
                ...
                wRet = TFCASTB_StopIPPVBuyDlg(CDCA_TRUE, wEcmPid,byPin,Price);
                switch(wRet)
                {
                        case CDCA_RC_OK;
                                printf("购买成功");
                ...
                TFCASTB_StopIPPVBuyDlg(CDCA_FALSE,wEcmPid,NULL,NULL);
	 */
	extern CDCA_U16 TFCASTB_StopIPPVBuyDlg(CDCA_BOOL bBuyProgram, CDCA_U16 wEcmPid, CDCA_U8* pbyPinCode, SCDCAIPPVPrice* pPrice);
...
//-------------------- 邮件管理 --------------------
	...
	例子：
        ...
        {
                byCount = 10;
                if(TFCASTB_GetEmailHeads(ArrEmailHead,&byCount,&byFromIndex) == CDCA_RC_OK)
                {
                        for(CDCA_U8 i = 0;i<byCount;i++)
                        {
                                ...
                                if(TFCASTB_GetEmailContent(ArrEmailHead[i].m_dwActionID,&EmailContent)==CDCA_RC_OK)
                                {
	...
	--*/
	extern CDCA_U16 TFCASTB_GetEmailHeads( SCDCAEmailHead* pEmailHead,  CDCA_U8* pbyCount,  CDCA_U8* pbyFromIndex);
	extern CDCA_U16 TFCASTB_GetEmailHead(CDCA_U32 dwEmailID,  SCDCAEmailHead* pEmailHead);
	extern CDCA_U16 TFCASTB_GetEmailContent(CDCA_U32 dwEmailID,  SCDCAEmailContent* pEmailContent);
		/*++
	说明 ：
		删除某一封邮件的方法在TFCASTB_GetEmailHeads使用介绍中已经介绍
		下面介绍删除所有邮件的方法 
	说明：
		使用TFCASTB_DelEmail来删除所有邮件，
	例子：
		TFCASTB_DelEmail(0);
	--*/
	extern void	TFCASTB_DelEmail(CDCA_U32 dwEmailID);
	/*++
	说明 ：
		获得信箱使用的情况
	例子：
				CDCA_U8    byEmailNum;
        CDCA_U8    byEmptyNum;
        if(TFCASTB_GetEmailSpaceInfo(&byEmailNum,&byEmptyNum) == CDCA_RC_OK)
        {
                printf("信箱中有邮件 ＝ %d个\n",byEmailNum);
                printf("信箱还可容纳邮件 ＝ %d个\n",byEmptyNum);
        }
	--*/
	extern CDCA_U16	TFCASTB_GetEmailSpaceInfo( CDCA_U8* pbyEmailNum,  CDCA_U8* pbyEmptyNum);
...
例子：
        ...
        CDCA_U8        data[256];
        CDCA_U8        len = 0;
        wRet= TFCASTB_ReadFeedDataFromParent(data,&len);
        if(wRet==CDCA_RC_OK)
                printf("获取母卡数据");
        }

 */
extern CDCA_U16  TFCASTB_ReadFeedDataFromParent(CDCA_U16 wTVSID,  CDCA_U8* pbyFeedData,  CDCA_U8* pbyLen);
/*
 说明 ：
	把从母卡读取的数据写入子卡，只对子卡有效
 参数：
    pFeedData：指向写入子卡的数据的空间，指向的数据是从调用TFCASTB_ReadFeedDataFromParent从母卡中返回的
        byLen：    写入子卡的数据长度 
例子：
        wRet= TFCASTB_WriteFeedDataToChild(data,len);
        if(wRet==CDCA_RC_OK)
                printf("写对应数据成功");
        }
 */
extern CDCA_U16  TFCASTB_WriteFeedDataToChild( CDCA_U8* pbyFeedData, CDCA_U8 byLen);
/*++
说明 ：
	获取运营商的子母卡状态
参数：
...
例子：
        ...
        CDCA_BOOL    bIsChild;
    		CDCA_U16   wOperatorID = 12345;
    		if(TFCASTB_GetOperatorChildStatus(wOperatorID,&bIsChild,&byDelayTime,&pLastFeedTime) == CDCA_RC_OK)
        {
                printf("获取运营商子母卡信息!);
        }
--*/
extern CDCA_U16 TFCASTB_GetOperatorChildStatus(CDCA_U16 wTVSID,  CDCA_U8* pbyIsChild,  CDCA_U8* pbyDelayTime,  CDCA_TIME* pLastFeedTime);
...
//-------------------- 线程管理 --------------------
extern CDCA_BOOL TFSTBCA_RegisterTask( const char* szName, CDCA_U8 byPriority,  void* pTaskFun,  void* pParam, CDCA_U16 wStackSize);
extern void      TFSTBCA_Sleep(CDCA_U16 wMilliSeconds);
...
调用的例子：
        ...
        TFSTBCA_SemaphoreInit(&Semaphore1,CDCA_TRUE);
        CDCA_Semaphore  Semaphore2;
        TFSTBCA_SemaphoreInit(&Semaphore2,CDCA_FALSE);
        结果 Semaphore1有信号
           　Semaphore2没有信号
--*/
extern void TFSTBCA_SemaphoreInit( CDCA_Semaphore* pSemaphore, CDCA_BOOL bInitVal);
extern void TFSTBCA_SemaphoreSignal( CDCA_Semaphore* pSemaphore );
extern void TFSTBCA_SemaphoreWait( CDCA_Semaphore* pSemaphore );

//-------------------- 内存管理 --------------------
/*++

--*/
extern void* TFSTBCA_Malloc(CDCA_U8 byBufSize);
extern void  TFSTBCA_Free(void* pBuf);
extern void  TFSTBCA_Memset(void* pDestBuf, CDCA_U8 c, CDCA_U16 wSize);
extern void  TFSTBCA_Memcpy( void* pDestBuf, const void* pSrcBuf, CDCA_U16 wSize );
...
机顶盒实现的例子:
		...
		TFSTBCA_GetBuffer( CDCA_U32* pdwStartAddr,CDCA_U32* pdwSize){
...
--*/
extern void TFSTBCA_GetBuffer(CDCA_U32* pdwStartAddr, CDCA_U32* pdwSize);
/*++
说明 ：
...
--*/
extern void TFSTBCA_ReadBuffer(CDCA_U32 dwStartAddr, CDCA_U8* pbyData, CDCA_U32* pdwLen);
/*++
说明 ：
    CA运行的过程中会多次写flash,每次的地址和长度都是一样的

--*/
extern void TFSTBCA_WriteBuffer(CDCA_U32 dwStartAddr, const CDCA_U8* pbyData, CDCA_U32 dwLen);
...
-*/
extern void	TFSTBCA_ScrSetCW(CDCA_U16 wEcmPID, const CDCA_U8* pbyOddKey, const CDCA_U8* pbyEvenKey, CDCA_U8 byKeyLen, CDCA_BOOL bTapingEnabled);
extern CDCA_BOOL TFSTBCA_GetPrivateData(CDCA_U8 byReqID, const CDCA_U8* pbyFilter, const CDCA_U8* pbyMask, CDCA_U8 byLen, CDCA_U16 wPid, CDCA_U8 byWaitSeconds);


//-------------------- 智能卡管理 --------------------
extern CDCA_BOOL TFSTBCA_SCReset(CDCA_U8* pbyATR,  CDCA_U8* pbyLen);
/*++
   在这里我举一个实现的模型 具体怎么做由机顶盒决定
 	
	  TFSTBCA_SCPBRun( const CDCA_U8* pbyCommand, CDCA_U16 wCommandLen,  CDCA_U8* pbyReply,  CDCA_U16* pwReplyLen)
        {
            命令字节送卡
            if(不成功)
                return CDCA_FALSE;
...
-*/
extern CDCA_BOOL TFSTBCA_SCPBRun( const CDCA_U8* pbyCommand, CDCA_U16 wCommandLen,  CDCA_U8* pbyReply,  CDCA_U16* pwReplyLen);

//-------------------- 授权信息管理 --------------------
/*当智能卡授权发生变化的时候，调用接口通知机顶盒*/
extern void TFSTBCA_EntitleChanged(CDCA_U16 wTvsID);

extern void TFSTBCA_DetitleReceived(CDCA_U16 wTvsID, CDCA_U8 bstatus);
...
*/
extern void     TFSTBCA_GetSTBID( CDCA_U16* pwPlatformID,  CDCA_U32* pdwUniqueID);
extern CDCA_U16 TFSTBCA_SCFunction( CDCA_U8* pData);


//-------------------- IPPV应用 --------------------
extern void TFSTBCA_HideIPPVDlg(CDCA_U16 wEcmPid);
extern void TFSTBCA_StartIppvBuyDlg(CDCA_U8 byMessageType, CDCA_U16 wEcmPid,  SCDCAIppvBuyInfo *pIppvProgram);

//-------------------- 邮件/OSD显示管理 --------------------
extern void TFSTBCA_ShowOSDMessage(CDCA_U8 byStyle,  char* szMessage);
extern void TFSTBCA_HideOSDMessage(CDCA_U8 byStyle);
/*++
 ...
 -*/
extern void TFSTBCA_EmailNotifyIcon(CDCA_U8 byShow, CDCA_U32 dwEmailID);
...
/*
 说明 ：
	请求机顶盒进入子卡喂养界面
*/
extern void  TFSTBCA_RequestFeeding(CDCA_U16 wTVSID);


//-------------------- 强制切换频道 --------------------
extern void TFSTBCA_LockService( SCDCALockService* pLockService);
extern void TFSTBCA_UNLockService(void);

//-------------------- 显示界面管理 --------------------
extern void TFSTBCA_ShowProgressStrip(CDCA_U8 byProgress, CDCA_U8 byMark);
extern void TFSTBCA_ShowBuyMessage(CDCA_U16 wEcmPID, CDCA_U8 byMessageType);
extern void TFSTBCA_ShowFingerMessage(CDCA_U16 wEcmPID, CDCA_U32 dwCardID);

//-------------------- 其它 --------------------
extern CDCA_U16 TFSTBCA_Strlen( char* pString);
extern void     TFSTBCA_Printf(CDCA_U8 byLevel,  const char* szMesssage);
