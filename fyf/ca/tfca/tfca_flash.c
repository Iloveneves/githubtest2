...
#define EEPROM_CDCA_FLASH_FLAG_ADDR (45)
#else
#define EEPROM_CDCA_FLASH_FLAG_ADDR (45)//(121)
#endif
#if 1
...
-------------------------------------------------------------------------------*/
void TFSTBCA_Flash_protect(void)
{
...
-------------------------------------------------------------------------------*/
void TFSTBCA_Flash_unprotect(void)
{
#ifdef 	CDCA_FLASH_CHECK_WRITE
...
-------------------------------------------------------------------------------*/
void TFSTBCA_ReadBuffer (CDCA_U32 dwStartAddr, CDCA_U8 * pbyData, CDCA_U32 * pdwLen)
{
...
-------------------------------------------------------------------------------*/
void TFSTBCA_IPI_WriteBuffer(void)
{
	FYF_FLS_IOCTL_PARAM_s FYF_FLS_IOCTL_param;
	FYF_E2P_IOCTL_PARAM_s fyf_e2p_ioctl_param;
...
-------------------------------------------------------------------------------*/
void TFSTBCA_WriteBuffer (CDCA_U32 dwStartAddr, const CDCA_U8 * pbyData, CDCA_U32 dwLen)
{
...
-------------------------------------------------------------------------------*/
void TFSTBCA_API_flash_thread(void *pvParam)
{
	while(1)
	{
		TFSTBCA_Flash_protect();
...
-------------------------------------------------------------------------------*/
void TFSTBCA_API_InitFlash(void)
{
#ifdef CDCA_FLASH_CHECK_WRITE
	cdca_flash_semaphore= FYF_API_semaphore_create("fla",1,FYF_THREAD_WAIT_PRIO);
	
	FYF_API_thread_create("fla",TFSTBCA_API_flash_thread,FYF_NULL, 15, 10 * 1024);
#endif
}
