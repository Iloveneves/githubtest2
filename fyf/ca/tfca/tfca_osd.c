...
//-------------------- 邮件/OSD显示管理 --------------------
void TFSTBCA_ShowOSDMessage(CDCA_U8 byStyle,  char* szMessage)
{
	FYF_IPI_ca_notify (FYF_CA_NOTIFY_SCROLL_MSG, (BU32 *)szMessage, (BU32 *)&byStyle);
	DFCA_DEBUG ("TFSTBCA_ShowOSDMessage byStyle=%d\n",byStyle);
}
void TFSTBCA_HideOSDMessage(CDCA_U8 byStyle)
{
//DFCA_DEBUG	 
	FYF_IPI_ca_notify (FYF_CA_NOTIFY_CLEAR_MSG, NULL, (BU32 *)&byStyle);
	DFCA_DEBUG ("TFSTBCA_HideOSDMessage byStyle=%d\n",byStyle);
}
//-------------------- 显示界面管理 --------------------
void TFSTBCA_ShowProgressStrip(CDCA_U8 byProgress, CDCA_U8 byMark)
{
	DFCA_DEBUG ("TFSTBCA_ShowProgressStrip byProgress=%d\n",byProgress);

	FYF_IPI_ca_notify (FYF_CA_NOTIFY_PROGRESS_STRIP, (BU32 *)&byProgress, (BU32 *)&byMark);
}

int byMessageNo_four = 0;
void TFSTBCA_ShowBuyMessage(CDCA_U16 wEcmPID, CDCA_U8 byMessageType)
{
	static BU32 messageNo = 0;
	DFCA_DEBUG ("TFSTBCA_ShowBuyMessage wEcmPID=%d\n",wEcmPID);
#if 0
...
void TFSTBCA_ShowFingerMessage(CDCA_U16 wEcmPID, CDCA_U32 dwCardID)
{
	BU32 wDuration;

	wDuration = wEcmPID;
	DFCA_DEBUG ("TFSTBCA_ShowFingerMessage wEcmPID=%d\n",wEcmPID);
...
/*++
 ...
 -*/
void TFSTBCA_EmailNotifyIcon(CDCA_U8 byShow, CDCA_U32 dwEmailID)
{
	switch(byShow)
	{
