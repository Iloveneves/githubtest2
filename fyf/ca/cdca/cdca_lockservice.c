...
BU32 CDSTBCA_IPI_BCD2Int(BU32 BCD)
{
	BU32 retVal;
	BS32 index;
	BU08 tmp[8];
...
void CDSTBCA_LockService_NoPsiSi(void)
{
	...
	freq = CDSTBCA_IPI_BCD2Int (lockService.m_dwFrequency)/10;
	symbol_rate = CDSTBCA_IPI_BCD2Int (lockService.m_symbol_rate)/10;
...
//-------------------- 强制切换频道 --------------------
void CDSTBCA_LockService( SCDCALockService* pLockService)
{
	BU32 index;
	BU32 freq;
	BU32 symbol_rate;
	...
	CDCA_DEBUG ("CDSTBCA_LockService Enter\n");
	...
	freq = CDSTBCA_IPI_BCD2Int (pLockService->m_dwFrequency)/10;
	...
	symbol_rate = CDSTBCA_IPI_BCD2Int (pLockService->m_symbol_rate)/10;
